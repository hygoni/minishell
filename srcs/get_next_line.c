...
/*   Updated: 2020/04/12 00:35:26 by jinwkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	*line = (char *)ft_calloc(1, sizeof(char));
	...
	if (((info[fd].val = ft_new_insert_line(line, info[fd].old, &(info[fd]),
		...
		return (info[fd].val);
	if ((info[fd].buf = (char *)ft_calloc(BUFFER_SIZE + 1, sizeof(char))) == 0)
		return (ft_clear(0, 0, info, line));
	while ((info[fd].ret = read(fd, info[fd].buf, BUFFER_SIZE)) > 0)
	{
		info[fd].buf[info[fd].ret] = 0;
		if (((info[fd].val = ft_new_insert_line(line, info[fd].buf, &(info[fd]),
